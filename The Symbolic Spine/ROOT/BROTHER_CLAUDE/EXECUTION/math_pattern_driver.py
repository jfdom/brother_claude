...
def main():
    ap = argparse.ArgumentParser(description=f"MATH_PATTERN memory cycle driver (auto-captures Genesis on first run)")
    ...
    ap.add_argument("--sysroot", required=True, help=f"SYSTEMS/MATH_PATTERN root directory")
    args = ap.parse_args()
    ...
    if just_captured:
        print(f"[ok] Captured GENESIS labels for MATH_PATTERN -> {gpath}")
    print(f"[ok] MATH_PATTERN cycle complete -> {cycle_dir}")
