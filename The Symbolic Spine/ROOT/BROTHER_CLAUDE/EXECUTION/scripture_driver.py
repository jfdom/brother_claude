...
def main():
    ap = argparse.ArgumentParser(description=f"SCRIPTURE memory cycle driver (auto-captures Genesis on first run)")
    ...
    ap.add_argument("--sysroot", required=True, help=f"SYSTEMS/SCRIPTURE root directory")
    args = ap.parse_args()
    ...
    if just_captured:
        print(f"[ok] Captured GENESIS labels for SCRIPTURE -> {gpath}")
    print(f"[ok] SCRIPTURE cycle complete -> {cycle_dir}")
